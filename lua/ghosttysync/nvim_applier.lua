...
function M.apply_highlights(highlight_map)
	...
	if not highlight_map or type(highlight_map) ~= "table" then
		error_msg = error_msg .. "Invalid highlight map provided\n"
		log_error(error_msg)
	...
	for group_name, group_config in pairs(highlight_map) do
		if type(group_name) ~= "string" then
			...
			error_msg = error_msg
				.. string.format("Invalid highlight group name (not a string): %s", tostring(group_name))
				.. "\n"
		...
		if group_name == "" then
			error_count = error_count + 1
			error_msg = error_msg .. "Empty highlight group name\n"
			-- table.insert(all_errors, error_msg)
			log_warning(error_msg)
			goto continue
		end
		...
		--   error_count = error_count + 1
		--   error_msg = string.format("Invalid config for highlight group '%s': %s", group_name, validated_config) .. "\n"
		--   -- table.insert(all_errors, error_msg)
		--   log_warning(error_msg)
		--   goto continue
		-- end
		...
		if apply_success then
			...
			error_count = error_count + 1
			error_msg = error_msg .. apply_error .. "\n"
	...
	if success_count > 0 then
		...
		error_msg = error_msg .. string.format("Failed to apply any highlight groups (%d errors)", error_count) .. "\n"
		log_error(error_msg)
		return false, error_msg
	end
end
...
function M.set_colorscheme_name(name)
	...
	local vim_api = get_vim_api()
	local success, result = M.handle_api_errors(function()
		vim_api.g.colors_name = name
	end, "setting colorscheme name")
...
function M.clear_existing_highlights()
	...
	local success1, result1 = M.handle_api_errors(function()
		vim_api.g.colors_name = nil
	end, "clearing colorscheme name")
	...
	local success2, result2 = M.handle_api_errors(function()
		vim_api.cmd("highlight clear")
	end, "clearing highlight groups")
	...
	local success3, result3 = M.handle_api_errors(function()
		if vim_api.fn.exists("syntax_on") == 1 then
			vim_api.cmd("syntax reset")
		end
	end, "resetting syntax highlighting")
