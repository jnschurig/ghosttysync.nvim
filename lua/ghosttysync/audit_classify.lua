...
function M.is_comment_class(name)
  ...
  if
    lname == "comment"
    or lname:match("^@comment")
    or lname == "lspinlayhint"
    or name == "SpecialComment"
    or name == "DiagnosticUnnecessary"
    or name == "DiagnosticDeprecated"
    or name == "Conceal"
    or name == "EndOfBuffer"
    or name == "NonText"
    or name == "Whitespace"
    or name == "Ignore"
    or name == "@lsp.type.comment"
  then
    return true
  end
  for _, p in ipairs(M.COMMENT_PATTERNS) do
    if lname:match(p) then
      return true
    end
  end
  return false
...
function M.classify(name, T)
  ...
  if
    name:match("^lualine_transitional_")
    or name:match("^lualine_.*_diff_")
    or name:match("^lualine_.*_diagnostics_")
    or name:match("^lualine_.*_filetype_MiniIcons")
  then
