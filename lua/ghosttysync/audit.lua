...
local classify = function(name, T)
  return cls.classify(name, T)
end
...
function M.run()
  ...
  local lines = {
    ...
    string.format(
      "Thresholds: TEXT=%.1f UI=%.1f COMMENT=%.1f FLOOR=%.1f",
      T.TEXT_MIN,
      T.UI_MIN,
      T.COMMENT_MIN,
      T.MIN_FLOOR
  ...
  for _, name in ipairs(names) do
    ...
    if cls and fg ~= bg then
      ...
      if ratio < cls.threshold then
        ...
        table.insert(
          lines,
          string.format("%-36s %-8s fg=%s bg=%s %.2f < %.2f", name:sub(1, 36), cls.kind, fg, bg, ratio, cls.threshold)
        )
      end
    end
  end
...
vim.api.nvim_create_user_command("GhosttysyncAudit", function()
  M.show()
end, {})
