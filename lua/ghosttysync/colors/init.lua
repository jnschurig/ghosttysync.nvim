...
colors.editor.border_strong = contrast.ensure_contrast(colors.editor.border, colors.editor.panel_bg, T.UI_MIN)
...
do
  ...
  for _, p in ipairs(pairs_to_check) do
    ...
    if a and b then
      colors.syntax[p[2]] = contrast.nudge_apart(a, b, colors.editor.bg, T.MIN_ROLE_DISTANCE, T.UI_MIN)
    end
  end
end
...
do
  ...
  local function collides(c)
    ...
    if contrast.wcag_ratio(text_fg, c) < T.TEXT_MIN then
      return true
    end
