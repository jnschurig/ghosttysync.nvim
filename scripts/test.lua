...
do
	...
	check("gamut clamp produces valid hex",
...
do
	...
	check("set_lightness changes L", approx(lch2.L, 0.8, 0.05),
		"got L=" .. lch2.L)
end
...
do
	...
	check("ensure_contrast meets threshold on dark bg", meets,
		string.format("before=%.2f after=%.2f", ratio_before, ratio_after))
	check("ensure_contrast no-op when already met",
		contrast.ensure_contrast("#ffffff", "#000000", 4.5) == "#ffffff")
end
...
do
	...
	check("nudge_apart increases distance", meets,
