...
local function classify(name)
  return cls.classify(name, THRESH)
end
...
local function audit_palette(fixture)
  ...
  if not normal_bg then
    normal_bg = fixture.colors.background
  end
  ...
  local panel_re =
    "Float.*Bg$|Popup.*Bg$|Cmdline.*Bg|^NormalFloat$|^NoiceCmdlinePopup$|^TelescopePromptNormal$|^SnacksPicker.*Normal$"
  for _, name in ipairs(names) do
    -- Lua patterns lack alternation; check each subpattern.
    local matched = name:match("Float.*Bg$")
      or name:match("Popup.*Bg$")
      or name:match("Cmdline.*Bg")
      or name == "NormalFloat"
      or name == "NoiceCmdlinePopup"
      or name == "TelescopePromptNormal"
  ...
  do
    ...
    local n_fg = (function()
      local f, _ = resolve_hl("Normal", normal_bg)
      return f
    end)()
...
local function print_report(rep)
  ...
  if #rep.failures == 0 then
    ...
    for _, r in ipairs(rep.failures) do
      print(
        string.format("  %-32s %-8s %-8s %-8s %-6.2f %.2f", r.name:sub(1, 32), r.kind, r.fg, r.bg, r.ratio, r.threshold)
      )
    end
  end
  ...
  if #rep.pair_failures == 0 then
    ...
    print(
      string.format("  distinguishability: %d collision(s) (min=%.2f)", #rep.pair_failures, THRESH.MIN_ROLE_DISTANCE)
    )
    for _, p in ipairs(rep.pair_failures) do
      print(string.format("    %s (%s) <-> %s (%s) dist=%.3f", p.a, p.a_color, p.b, p.b_color, p.distance))
    end
  end
  ...
  if #rep.mode_bg_failures == 0 then
    ...
    print(
      string.format(
        "  mode_bg_distinguishability: %d collision(s) (min=%.2f)",
        #rep.mode_bg_failures,
        THRESH.MIN_ROLE_DISTANCE
      )
    )
  ...
  if #rep.border_failures == 0 then
    ...
    print(
      string.format("  floating_border_visibility: %d failure(s) (min ratio=%.2f)", #rep.border_failures, THRESH.UI_MIN)
    )
  ...
  if #rep.panel_bg_failures == 0 then
    ...
    print(
      string.format(
        "  floating_panel_bg_distinct: %d failure(s) (min ΔE=%.2f)",
        #rep.panel_bg_failures,
        THRESH.PANEL_BG_OFFSET
      )
    )
    for _, p in ipairs(rep.panel_bg_failures) do
      print(string.format("    %-32s bg=%s normal=%s ΔE=%.3f", p.name:sub(1, 32), p.bg, p.normal_bg, p.distance))
    end
  end
  ...
  if #rep.coverage_failures == 0 then
    ...
    print(string.format("  palette_coverage: %d unused index/indices", #rep.coverage_failures))
    for _, c in ipairs(rep.coverage_failures) do
      print(string.format("    palette[%d] = %s  (no applied highlight references it)", c.index, c.color))
    end
  end
end
